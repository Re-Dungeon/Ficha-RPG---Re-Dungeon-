...
**As 8 fases do roadmap (seção 10) foram implementadas.** Falta apenas o deploy final no Firebase Hosting, adiado a pedido do responsável pelo projeto — o código está pronto para isso quando for disparado (`npm run build && firebase deploy --only hosting`, após um `firebase init` hosting apontando para `dist/`).
...
A ficha do personagem (`src/pages/Personagens/Ficha/Ficha.jsx`) acabou com **10 abas**: Atributos, Progressão, Aptidões, Arts, Inventário, Lojas, Veias Astrais, Sorte, Condições, Códex, Perfil.
...
- **Inventário unificado**: uma lista só (`inventario: [{itemId, quantidade, equipado}]`), sem a separação "Itens" vs. "Armazenamentos" do site vanilla — um item vira "armazenamento" se o catálogo tiver o campo `bonusEspaco`. Consistente com o schema já simplificado da seção 5.
...
- **Arts**: modelo "flat" — um único documento por Art com `origem: 'autoral'|'classe'|'catalogo'`, sem entidades separadas de Núcleo/Variante como o site vanilla tinha. É literalmente o que a seção 5.1 já descreve, só não estava explícito que a hierarquia de 3 camadas do vanilla não seria replicada.
...
- **Reputação (Fama/Temor) não será implementada nesta fase.** Documentada em `FUNCIONALIDADES.md` §5 como referência do comportamento atual, mas fora do roadmap abaixo.
...
|---|---|
| Reputação | Decisão de produto — não implementar nesta fase |
...
| `materiais`, `receitas`, `cardflux` | — | Coleções existentes **sem funcionalidade correspondente** em `FUNCIONALIDADES.md` — provavelmente sistemas futuros (crafting, um jogo de cartas?) fora do escopo atual. Não assumir mapeamento nem duplicar; apenas não usar por enquanto |

### 4.1 O campo `universo` é a chave de escopo transversal — **confirmado**
`racas`, `classes`, `condicoes`, `aptidoes`, `veiasAstrais`, `itens` e `artes` são **todas** filtradas pelo `universo` do personagem antes de aparecer como opção — esse é o mesmo conceito de "universo temático" que hoje agrupa Raças/Classes em pastas no site vanilla (Re'Geron, The Chaotical Gate, Cultivo, One Piece, Bleach, A Crônica dos Varkhan). `divindades`, `origens` e `regras` não são mencionadas como filtradas — tratar como compartilhadas entre universos, salvo indicação em contrário do painel administrativo.
...
```js
{
  ...
  inventario: [ { itemId: string, quantidade: number } ],   // referencia `itens`
  ...
  // reputacao: { mundo: {fama, temor}, regiao: {fama, temor} },
  ...
  // companheiros: -> ver subcoleção comentada em 5.1
...
personagens/{personagemId}/arts/{artId}
  ...
  // se origem === 'autoral': campos completos de Núcleo/Art/Variante (tipo, domínio, recarga, duração,
  //   alcance, alvos, custo, dano, ação, status), como hoje em `redungeon_character`
  // se origem === 'classe': { classeId, habilidadeId } — referencia a habilidade já definida em `classes`
  // se origem === 'catalogo': { arteId } — referencia um documento em `artes` (filtrado por `universo` na hora de escolher)

personagens/{personagemId}/historicoSorte/{eventoId}
  tipo, descricao, valor, timestamp

// reservado para quando Companheiros voltar ao roadmap (seção 2.3) — não criar ainda:
// personagens/{personagemId}/companheiros/{companheiroId}
```
...
- Deploy (Firebase Hosting, como o restante da família de projetos).
...
- ✅ Reputação, Corpo Imortal, Companheiros e Cultivação ficam fora desta fase (seção 2).
